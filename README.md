...
| **MazeMobber** | Solves the shuffling mazes in twenty-one areas and walks you to what matters in them: **Prosper** (the 201 theater and the Sea Witch), **Gauntlet**, **Gwillimberry**, **Nenukon**, **Xylmos**, **Helegear**, **Infamy**, **Sahuagin**, **Darklight**, **Times of Old**, **Promises**, **Rhodus**, **Astral**, **Fairy Tales 1**, **Scarred Lands**, **Nottingham**, **Deadlights**, **Mayhem**, **Sundered Vale**, **Gallows** and **Gold Rush**. Three commands: **`mm`** puts up a clickable list for whichever one you are standing in — mobs to hunt, rooms to walk to — and it walks the whole way; **`mmr`** picks a run back up after a fight or a warded door stops it; and **`mmout`** (or `mmo`) leaves by the nearest way out. The list that comes up when you walk in opens in a small scrolling window (`mm window` switches it back to the main window, `mm win` reopens it any time; it closes itself when you leave the area, and `mm font` / `mm fontsize` change its font); `mm list` stops an area putting its list up on arrival. Outside all of them, `mm` lists the areas it works in - click one to be taken there. Scans where a mob scans, reads the room where it does not, never walks into a wall or out of the maze, and only searches the rooms a mob actually uses. `mm help` lists every area and the word that drives it. |
