...
| **Gauntlet** | Solves the shuffling Gauntlet maze and hunts any mob in it (`gaunt`). Scans as it sweeps and walks you straight to the mob the moment scan picks it up; the two noscan mobs (giant skeleton, night hunter bat) are found by reading every room's character list instead. Clickable mob list that pops up on its own when you enter the maze, plus the Pre-Battle / BEFORE FIGHT / BATTLE rooms as destinations. |
| **STTNG** | Solves the shuffling level 201 pk maze above The Imaginarium on Prosper's Island and walks you to any room in it (`sttng`, or `sttng3`). Clickable room list that pops up when you walk into Entering the Theater, including the four side rooms (Stalwart, Brawn, Moriarty, The Bridge), plus `sttngout` to leave. The maze reshuffles every repop and the plugin re-solves it on its own. |
| **Garden** | Walks the Botanical Gardens maze in Gwillimberry looking for the Gwillim mobs (`gard`). Six of the seven are noscan and none of them print their short name in the room, so each is matched on a phrase from its room description instead; only the nibbling rabbit is found by scanning. Clickable list that pops up when you reach the gate, and `gardout` to walk back and out through the south door. ⚠️ New — still needs live testing. |
