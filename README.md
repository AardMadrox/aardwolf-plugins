...
| **MazeMobber** | Solves the shuffling mazes in nineteen areas and walks you to what matters in them: **Prosper** (the 201 theater), **Gauntlet**, **Gwillimberry**, **Nenukon**, **Xylmos**, **Helegear**, **Infamy**, **Sahuagin**, **Darklight**, **Times of Old**, **Promises**, **Rhodus**, **Astral**, **Fairy Tales 1**, **Scarred Lands**, **Nottingham**, **Deadlights**, **Mayhem** and **Sundered Vale**. Three commands: **`mm`** puts up a clickable list for whichever one you are standing in — mobs to hunt, rooms to walk to — **`mmg`** carries on, and **`mmout`** (or `mmo`) leaves by the nearest way out. It walks the whole way by default; `mmslow` makes it one room per press, `mmr` resumes after a fight, `mm list` stops an area putting its list up on arrival. Scans where a mob scans, reads the room where it does not, reads the exit listing where a room names what its exits open on, and only searches the rooms a mob actually uses. `mm help` lists every area and the word that drives it. |
