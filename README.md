...
| **MazeMobber** | Solves the shuffling mazes in five areas and finds what is in them: **Gauntlet**, **Gwillimberry** (Botanical Gardens), **Nenukon**, **Xylmos** (Rose Ether) and **Helegear**. Use `gaunt`, `gard`, `nenukon`, `xyl` or `hele` for a specific one, or **`mm`** for whichever you are standing in — anywhere in those areas counts, and it will walk you to the maze entrance first. Clickable mob list per area, scanning where a mob scans and reading the room where it does not, with each mob only searched in the rooms it actually uses. **`mmout`** (or `gardout` / `nenukonout` / `xylout` / `heleout`) leaves by the nearest way out. Stops if you are attacked and offers a one-click resume. |
